...
class Present:
    def __init__(self):
        #size
        random_size_value = random.uniform(PRESENT_SIZE_RANDOMIZE[0], PRESENT_SIZE_RANDOMIZE[1])
        size = (int(PRESENTS_SIZES[0] * random_size_value), int(PRESENTS_SIZES[1] * random_size_value))
        # moving
        moving_direction, start_pos = self.define_spawn_pos(size)
        # sprite
        self.rect = pygame.Rect(start_pos[0], start_pos[1], size[0]//1.4, size[1]//1.4)
        self.images = [image.load("Assets/present/present.png", size=size, flip=moving_direction=="right")]
        self.current_frame = 0
        self.animation_timer = 0


    def define_spawn_pos(self, size): # define the start pos and moving vel of the present
        vel = random.uniform(PRESENTS_MOVE_SPEED["min"], PRESENTS_MOVE_SPEED["max"])
    ...
    def kill(self, PRESENTS): # remove the present from the list
